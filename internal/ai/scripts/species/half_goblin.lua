-- half_goblin.lua
-- Half-Goblin AI: less disciplined than full goblins but still
-- communist in nature — they share loot and cooperate in groups,
-- though they are more individualistic than pure goblins.
        ...
        if eid ~= self.id then
            ...
            if info and info.alive and info.faction == self.faction and info.hp < info.max_hp then
                if world.has_material("raw_meat") then
                    ...
                    util.log(self.name .. " shares with " .. info.name .. " (half-goblin solidarity)")
                    return
                end
            end
        end
