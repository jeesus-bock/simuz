-- orc.lua
-- Communist orc species script: collective ownership, shared resources,
-- group defense, and redistribution of loot. Orcs work as a collective
-- where all gains are shared and private hoarding is shunned.
...
local function share_resources()
    -- Redistribute any excess items to wounded allies
    local nearby = world.nearby_entities()
    ...
    for _, eid in ipairs(nearby) do
        if eid ~= self.id then
            ...
            if info and info.alive and info.faction == self.faction and info.hp < info.max_hp then
                -- Share food/health items with injured allies
                if world.has_material("raw_meat") then
                    world.give_item(eid, "raw_meat")
                    util.log(self.name .. " shares a portion of the collective's meat with " .. info.name)
                    return
                end
            end
        end
...
function do_tick()
    ...
    -- Night: return to the collective camp
    if phase == "night" then
        if self.home and self.loc_id ~= self.home then
            world.move_to(self.home)
            return {util.event("move", {})}
        end
        -- Rest and recover with the collective
        if tick % 10 == 0 then
            share_resources()
        end
        return {}
    ...
    -- Priority 3: Attack hostiles as a collective
    if tick % 6 == 0 then
    ...
    -- Priority 4: Wander and scout for threats or resources together
