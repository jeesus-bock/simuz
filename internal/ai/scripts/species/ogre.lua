-- ogre.lua
-- Communist ogre AI: forced collective labor, communal territory
-- defense, shared food stores. Ogres work under collective
-- direction — the strongest ogre leads but all share equally.
...
local function share_food()
    -- Ogres share food from the communal store
    local nearby = world.nearby_entities()
        ...
        if eid ~= self.id then
            ...
            if info and info.alive and info.faction == self.faction and info.hp < info.max_hp then
                if world.has_material("raw_meat") then
                    ...
                    util.log(self.name .. " distributes collective rations to " .. info.name)
                    return
                end
            end
        end
...
local function collective_defense()
    -- Ogres defend territory as a collective wall
    ...
    if hit then
        util.log(self.name .. " crushes " .. (target_info.name or target_id) .. " for the collective!")
        -- Distribute any loot
...
local function defend_collective()
    -- Protect the weakest ogre in the collective
...
local function coordinate_labor()
    -- Ogres coordinate collective labor: building, foraging, fortifying
    local allies = count_allies()
    if allies >= 2 then
        util.log(self.name .. " directs " .. allies .. " ogres in collective labor")
...
function do_tick()
    ...
    -- Night: return to the communal camp
    if phase == "night" then
        if self.home and self.loc_id ~= self.home then
            world.move_to(self.home)
            return {util.event("move", {})}
        end
        -- Share food with the collective at night
        if tick % 10 == 0 then
            share_food()
        end
        return {}
    ...
    -- Day: collective labor and defense
    ...
    -- Priority 2: Share food with the collective
    if tick % 15 == 0 then
        share_food()
    end

    -- Priority 3: Coordinate collective labor
    if tick % 20 == 0 then
        coordinate_labor()
    end

    -- Priority 4: Collective territory defense
    if tick % 8 == 0 then
    ...
    -- Priority 5: Patrol the territory
