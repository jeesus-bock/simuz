...
// Returns nil if the species is not registered.
...
func SpeciesMaxAge(species string) int {
	if s, ok := SpeciesRegistry[species]; ok {
		return s.MaxAge
	}
	return 0
}
...
// IsImmortalSpecies returns whether a species is immortal (cannot die of old age).
func IsImmortalSpecies(species string) bool {
	if s, ok := SpeciesRegistry[species]; ok {
		return s.IsImmortal
	}
	return false
