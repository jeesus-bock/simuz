...
func (g *Generator) generateRatKingLair() []*entity.Entity {
	...
	var all []*entity.Entity
	for i := 0; i < 2; i++ {
		rat := entity.NewEntity("rat_scram"+fmt.Sprint(i), "Scram", "rat", ratAttrs1, 1)
	...
	for i := 0; i < 2; i++ {
		rat := entity.NewEntity("rat_gleam"+fmt.Sprint(i), "Gleam", "rat", ratAttrs2, 2)
	...
	for i := 0; i < 2; i++ {
		scout := entity.NewEntity("rat_scout"+fmt.Sprint(i), "Rat_Scout", "rat", ratAttrs1, 1)
	...
	ratKing := entity.NewEntity("rat_king", "Skreet the Unseen", "rat_king", entity.Attributes{STR: 18, DEX: 10, CON: 20, INT: 8, WIS: 12, CHA: 6}, 8)
...
func (g *Generator) generateFisherman() []*entity.Entity {
	...
	fisher := entity.NewEntity("stillwater_fisher", "Oswin", "human", entity.RandomAttributes(g.RNG.Intn), 2)
...
func (g *Generator) generateNewArchetypes() []*entity.Entity {
	...
	healer := entity.NewEntity("fey_healer", "Willow", "fey", healerAttrs, 3)
	...
	badger := entity.NewEntity("badger_defensive", "Brutus", "badger", badgerAttrs, 2)
...
func (g *Generator) generateNewCreatures() []*entity.Entity {
	...
	vampire := entity.NewEntity("vampire_valerius", "Count Valerius", "vampire", vampireAttrs, 5)
	...
	for ti, town := range towns {
		...
		for _, ad := range animalDefs {
			for i := 0; i < ad.count; i++ {
				...
				animal := entity.NewEntity(animalID, ad.species+"_"+fmt.Sprint(i), ad.species, ad.attrs, 1)
...
func (g *Generator) generateTownExtras() []*entity.Entity {
	...
	for i, townID := range townIDs {
		...
		miner := entity.NewEntity(townID+"_miner", minerNames[i], "human", entity.RandomAttributes(g.RNG.Intn), 2)
		...
		herbalist := entity.NewEntity(townID+"_herbalist", "Herbalist "+fmt.Sprint(i+1), "human", entity.RandomAttributes(g.RNG.Intn), 2)
...
func (g *Generator) generateWildernessBosses() []*entity.Entity {
	...
	for i, name := range cultistNames {
		cultist := entity.NewEntity("cultist_"+fmt.Sprint(i), name, "human", entity.RandomAttributes(g.RNG.Intn), 2+g.RNG.Intn(2))
	...
	werewolf := entity.NewEntity("werewolf_cursed", "Cursed Traveler", "human", entity.Attributes{STR: 16, DEX: 14, CON: 14, INT: 8, WIS: 10, CHA: 8}, 4)
	...
	necromancer := entity.NewEntity("necromancer_morth", "Morth the Pale", "human", entity.Attributes{STR: 10, DEX: 12, CON: 12, INT: 18, WIS: 14, CHA: 10}, 5)
	...
	dragon := entity.NewEntity("dragon_ash", "Ashscale the Ancient", "dragon", entity.Attributes{STR: 30, DEX: 16, CON: 28, INT: 14, WIS: 16, CHA: 20}, 10)
	...
	for i, loc := range rangerLocs {
		ranger := entity.NewEntity("ranger_"+fmt.Sprint(i), rangerNames[i], "human", entity.Attributes{STR: 14, DEX: 16, CON: 12, INT: 10, WIS: 14, CHA: 10}, 3)
	...
	banditChief := entity.NewEntity("bandit_chief", "Blackheart", "human", entity.Attributes{STR: 18, DEX: 14, CON: 16, INT: 10, WIS: 10, CHA: 12}, 5)
...
func (g *Generator) generateGoblinAmbushers() []*entity.Entity {
	...
	for i := 0; i < 2; i++ {
		goblin := entity.NewEntity("goblin_ambush_"+fmt.Sprint(i), "GoblinAmbush"+fmt.Sprint(i), "goblin", goblinAttrs, 1+g.RNG.Intn(2))
