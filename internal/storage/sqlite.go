...
func (s *SQLiteStore) migrate() error {
	...
	alterQueries := []string{
		...
		"ALTER TABLE entities ADD COLUMN cause TEXT DEFAULT ''",
	}
...
func (s *SQLiteStore) Save(sim *engine.Simulation) error {
	...
	for _, ent := range sim.Entities.All() {
		...
		_, err = tx.Exec(`INSERT OR REPLACE INTO entities (id, name, species, level, age, max_age, last_meal, alive, attrs_json, skills_json, hp, max_hp, fp, max_fp, xp, location_id, pos_x, pos_y, ai_json, faction, flags_json, inventory_json, equipment_json, effects_json, cause) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (s *SQLiteStore) Load() (*engine.Simulation, error) {
	...
	entRows, err := s.db.Query(`SELECT id, name, species, level, age, max_age, last_meal, alive, attrs_json, skills_json, hp, max_hp, fp, max_fp, xp, location_id, pos_x, pos_y, ai_json, faction, flags_json, inventory_json, equipment_json, effects_json, COALESCE(cause,'') FROM entities`)
	if err != nil {
	...
	for entRows.Next() {
		...
		var attrsJSON, skillsJSON, aiJSON, faction, flagsJSON, inventoryJSON, equipmentJSON, cause string
		...
		if err := entRows.Scan(&id, &name, &species, &level, &age, &maxAge, &lastMeal, &alive, &attrsJSON, &skillsJSON,
			...
			&aiJSON, &faction, &flagsJSON, &inventoryJSON, &equipmentJSON, &effectsJSON, &cause); err != nil {
